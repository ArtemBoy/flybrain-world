...
let heading=0,mode="HUMAN",brainTimer=0;
function reset(){agent.position.set(0,.35,0);heading=0;target.position.x=2+Math.random()*3;target.position.z=-3+Math.random()*6}
function bearing(){const dx=target.position.x-agent.position.x,dz=target.position.z-agent.position.z,t=Math.atan2(dx,-dz);return Math.atan2(Math.sin(t-heading),Math.cos(t-heading))}
function distance(){return Math.hypot(target.position.x-agent.position.x,target.position.z-agent.position.z)}
function act(action){if(action==="LEFT")heading+=.12;if(action==="RIGHT")heading-=.12;if(action==="FORWARD"){agent.position.x+=Math.sin(heading)*.12;agent.position.z-=Math.cos(heading)*.12}agent.rotation.y=heading;["left","forward","right"].forEach(x=>document.querySelector("#"+x).value=x.toUpperCase()===action?1:0)}
function brainStep(){const b=bearing(),a=Math.abs(b)<.18?"FORWARD":(b>0?"LEFT":"RIGHT");act(a);document.querySelector("#brain-status").textContent=`ToyBrain output: ${a} · bearing ${b.toFixed(2)} rad`}
...
addEventListener("keydown",e=>{if(e.key.toLowerCase()==="b")setMode("BRAIN");if(e.key.toLowerCase()==="h")setMode("HUMAN");if(e.key.toLowerCase()==="r")reset();if(mode==="HUMAN"){if(e.key==="ArrowUp"||e.key.toLowerCase()==="w")act("FORWARD");if(e.key==="ArrowLeft"||e.key.toLowerCase()==="a")act("LEFT");if(e.key==="ArrowRight"||e.key.toLowerCase()==="d")act("RIGHT")}});
